...
import * as occupationsDb from "./occupationsDb"
import {duration} from "moment";

/**
 * Occupations file
 */
const FILE_OCCUPATIONS = "occupations.list.json";

/**
 * Takes occupations source files and populates SQLite database
 * @param schemaPath Where the Room schemas are put
 * @param dataDir Where the data is put
 * @return A promise for created database
 */
export async function occupations(schemaPath: string, dataDir: string): Promise<string> {
    ...
    await populate(
        ...
        async function(this: Database): Promise<void> {
            await occupationsDb.populate(
                db,
                await readJsonData<Array<Occupation>>(Path.join(dataDir, FILE_OCCUPATIONS))
            );
        }
    );
